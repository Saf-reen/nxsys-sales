...
const StarRating = ({ value = 0, onChange = undefined as ((r: number) => void) | undefined, size = 18 }) => {
...
function ProductReviews({ productId }) {
  ...
  const [editingReviewId, setEditingReviewId] = useState<any>(null);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const isDuplicate = 
        ...
        String((err as any)?.response?.data || '').toLowerCase().includes('unique_product_user_review');
