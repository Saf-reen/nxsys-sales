...
const writeGuestWishlist = (ids: string[] = []) => {
  if (!canUseStorage()) {
    return;
  }
...
export function WishlistProvider({ children }) {
  const [productIds, setProductIds] = useState<string[]>(() => readGuestWishlist());
  ...
  const syncAuthenticatedWishlist = useCallback(async () => {
    ...
    try {
      ...
      // Extract IDs from the nested products array if it's an object with products, 
      // otherwise fallback to the raw response if it was already an array.
      const remoteIds = Array.isArray(response) 
        ? response.map(String) 
        : (response?.products?.map((p: any) => String(p.id)) || []);

      const mergedIds = uniqueIds([...remoteIds, ...guestIds]);

      if (guestIds.length && mergedIds.length !== remoteIds.length) {
        const response = await wishlistApi.setWishlist(mergedIds);
        const savedIds = Array.isArray(response) 
          ? response.map(String) 
          : (response?.products?.map((p: any) => String(p.id)) || mergedIds);
  ...
  const persistAuthenticatedIds = useCallback(async (nextIds, actionType = 'set', targetProductId = null) => {
    ...
    try {
      ...
      if (actionType === 'add' && targetProductId) {
        ...
        const response = await wishlistApi.setWishlist(optimisticIds);
        resolvedIds = Array.isArray(response) 
          ? response.map(String) 
          : (response?.products?.map((p: any) => String(p.id)) || optimisticIds);
      }
