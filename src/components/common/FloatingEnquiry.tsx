...
function FloatingEnquiry() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await ordersApi.submitEnquiry(submissionData);
      setIsSuccess(true);
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-5 right-5 z-[90] flex items-center gap-2.5 rounded-full bg-textMain pl-4 pr-5 py-3 text-primary shadow-[0_8px_32px_rgba(0,0,0,0.3)] transition-all hover:scale-[1.04] hover:shadow-[0_12px_40px_rgba(0,0,0,0.35)] active:scale-95 sm:bottom-8 sm:right-8"
        aria-label="Enquiry Form"
      >
        <MessageCircle size={20} />
        <span className="hidden text-[11px] font-black uppercase tracking-widest sm:block">Enquiry</span>
        <span className="flex h-4 w-4 shrink-0 animate-pulse items-center justify-center rounded-full bg-primary text-[9px] font-black text-textMain">
          !
        </span>
      </button>
      ...
      {/* Modal Overlay */}
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/60 p-0 backdrop-blur-sm animate-in fade-in duration-300 sm:items-center sm:p-4">
          <div
            className="relative w-full max-w-lg overflow-hidden rounded-t-[32px] bg-white shadow-2xl animate-in slide-in-from-bottom duration-300 sm:rounded-[32px] sm:animate-in sm:zoom-in-95"
            role="dialog"
            aria-modal="true"
          >
            {/* Header */}
