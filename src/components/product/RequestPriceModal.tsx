  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        product: product.id, // Only send the ID, not the whole object
      };
      ...
      try {
        ...
        const isServerCrash = (error as any)?.response?.status === 500;
        ...
        if (isServerCrash) {
          ...
          try {
            ...
            const matched = recentRequests.slice(0, 3).find(r => 
              ...
              (new Date().getTime() - new Date(r.createdAt || 0).getTime()) < 120000 // Within last 2 mins
            );
